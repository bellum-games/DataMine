...
namespace DataMine
{
    public partial class DataMine : Form
    {
        public DataMine()
        {
            InitializeComponent();
        ...
        Dictionary<string, DataStruct> vectorGlobal = new Dictionary<string, DataStruct>();
        ...
        private void openPathToolStripMenuItem_Click(object sender, EventArgs e)//Open Path
        {
        ...
        private void doThingsToolStripMenuItem_Click(object sender, EventArgs e)//Do Things
        {
            ...
            {
                XmlTextReader reader = new(file.FullName);
                ...
                //Data = ClearStopWords(Data, fileContent); //Functie functionala dar hapsana :)))
                string[] spargeContinutulUnuiFisier = Data.Split(' ');
                Array.Sort(spargeContinutulUnuiFisier);
                foreach (string continutFisier in spargeContinutulUnuiFisier)
                {
                    if (!vectorGlobal.ContainsKey(continutFisier))//daca cuvantul curent NU se gaseste in dictionarul vector global atunci adauga-l
                    {
                        ;
                        vectorGlobal.TryAdd(stem(continutFisier), new DataStruct(file.Name, 1));
                    }
                    else //daca cuvantul curent e deja in dictionar atunci crestei frecventa
                    {
                        vectorGlobal[continutFisier].setFrecventa(vectorGlobal[continutFisier].getFrecventa() + 1);
                    }
                
                }
                
                Data = string.Empty;//Recontruieste Data in ordine alfabetica dupa sortare si prelucrare
                foreach (string reconstruiesteData in spargeContinutulUnuiFisier)
                {
                   
                    Data = Data + reconstruiesteData + " ";
                }//Recontruieste Data in ordine alfabetica dupa sortare si prelucrare
                Everything = Everything + " " + Data + Environment.NewLine + Environment.NewLine;
            }
            mainBox.Text = Everything;
        }

        private void seeTheGlobalVectorToolStripMenuItem_Click(object sender, EventArgs e)//See The Global Vector
        {
            secondaryBox.Text = string.Empty;

            foreach (var dic in vectorGlobal)
            {

                secondaryBox.Text = secondaryBox.Text + "Cuvantul meu e: " + dic.Key + " din fisierul: " + dic.Value.getFisier() + " si are frecventa: " + dic.Value.getFrecventa().ToString() + Environment.NewLine;
            }
        }

        private static string ClearStopWords(string Data, string fileContent)
        {
            string[] stopwords = fileContent.Split(Environment.NewLine);
        ...
        string[] step4Replacements = { "ement", "ment", "able", "ible", "ance", "ence", "ate", "iti", "ion", "ize", "ive", "ous", "ant", "ism", "ent", "al", "er", "ic" };
        ...
        string[] exceptions2 = { "inning", "outing", "canning", "herring", "earring", "proceed", "exceed", "succeed" };
